...
ggsave(
  filename = paste0(path_to_save, "traceplot_m1.png"),
  plot = traceplot_m1,
  width = 9.46,
  height = 6.27,
  dpi = 300
...
y_predict_m1_df <-
  data.frame(y1_pred_m1 = y_predict_m1$y1_tilde[1, ],
             y2_pred_m1 = y_predict_m1$y2_tilde[1, ])

brasileirao_2021_with_pred <-
  cbind(brasileirao_2021, y_predict_m1_df)
...
ggsave(
  filename = paste0(path_to_save, "cumsum_m1.png"),
  plot = cumsum_m1,
  width = 9.76,
  height = 7.37,
  dpi = 300
...
pontuacao <- points_per_game_obs_m1 |>
  group_by(team_name) |>
  summarize(
    score_obs = sum(points_scored_obs),
...
mse_m1 <- data.frame(
  mse_y1 = mse(
    brasileirao_2021_with_pred$y1,
    brasileirao_2021_with_pred$y1_pred_m1
  ),
  mse_y2 = mse(
    brasileirao_2021_with_pred$y2,
...
brasileirao_2021_list_m2 <- list(
  G = nrow(brasileirao_2021),
  T = 20,
  h = brasileirao_2021$h,
  a = brasileirao_2021$a,
  y1 = brasileirao_2021$y1,
...
ggsave(
  filename = paste0(path_to_save, "traceplot_m2.png"),
  plot = traceplot_m2,
  width = 9.46,
  height = 6.27,
  dpi = 300
...
y_predict_m2_df <-
  data.frame(y1_pred_m2 = y_predict_m2$y1_tilde[1, ],
             y2_pred_m2 = y_predict_m2$y2_tilde[1, ])

brasileirao_2021_with_pred <-
  cbind(brasileirao_2021_with_pred, y_predict_m2_df)

score_brasileirao_2021_m2_pred <-
  get_team_points_per_game(data = brasileirao_2021_with_pred,
                           y1 = quo(y1_pred_m2),
                           y2 = quo(y2_pred_m2))

points_per_game_obs_m2 <- merge(
  score_brasileirao_2021,
  score_brasileirao_2021_m2_pred,
  by = c("game_id", "team_id", "team_name"),
...
ggsave(
  filename = paste0(path_to_save, "cumsum_m2.png"),
  plot = cumsum_m2,
  width = 9.76,
  height = 7.37,
  dpi = 300
...
mse_m2 <- data.frame(
  mse_y1 = mse(
    brasileirao_2021_with_pred$y1,
    brasileirao_2021_with_pred$y1_pred_m2
  ),
  mse_y2 = mse(
    brasileirao_2021_with_pred$y2,
    brasileirao_2021_with_pred$y2_pred_m2
  )
)
...
ggsave(
  filename = paste0(path_to_save, "traceplot_m3.png"),
  plot = traceplot_m3,
  width = 9.46,
  height = 6.27,
  dpi = 300
...
y_predict_m3_df <-
  data.frame(y1_pred_m3 = y_predict_m3$y1_tilde[1, ],
             y2_pred_m3 = y_predict_m3$y2_tilde[1, ])

brasileirao_2021_with_pred <-
  cbind(brasileirao_2021_with_pred, y_predict_m3_df)

score_brasileirao_2021_m3_pred <-
  get_team_points_per_game(data = brasileirao_2021_with_pred,
                           y1 = quo(y1_pred_m3),
                           y2 = quo(y2_pred_m3))

points_per_game_obs_m3 <- merge(
  score_brasileirao_2021,
  score_brasileirao_2021_m3_pred,
  by = c("game_id", "team_id", "team_name"),
...
ggsave(
  filename = paste0(path_to_save, "cumsum_m3.png"),
  plot = cumsum_m3,
  width = 9.76,
  height = 7.37,
  dpi = 300
...
mse_m3 <- data.frame(
  mse_y1 = mse(
    brasileirao_2021_with_pred$y1,
    brasileirao_2021_with_pred$y1_pred_m3
  ),
  mse_y2 = mse(
    brasileirao_2021_with_pred$y2,
    brasileirao_2021_with_pred$y2_pred_m3
  )
)
...
ggsave(
  ...
  y_predict_m4 <-
    extract(m4_fit, pars = c("y1_tilde", "y2_tilde"))
  y_predict_m4_df <-
    data.frame(
      y1_pred_m4 = y_predict_m4$y1_tilde[1, ],
      y2_pred_m4 = y_predict_m4$y2_tilde[1, ]
    )
  
  brasileirao_2021_with_pred <-
    cbind(brasileirao_2021_with_pred, y_predict_m4_df)
  
  score_brasileirao_2021_m4_pred <-
    get_team_points_per_game(
      data = brasileirao_2021_with_pred,
      y1 = quo(y1_pred_m4),
      y2 = quo(y2_pred_m4)
    )
  
  points_per_game_obs_m4 <- merge(
    score_brasileirao_2021,
    score_brasileirao_2021_m4_pred,
    by = c("game_id", "team_id", "team_name"),
  ...
  ggsave(
    filename = paste0(path_to_save, "cumsum_m4.png"),
    plot = cumsum_m4,
    width = 9.76,
    height = 7.37,
    dpi = 300
  ...
  mse_m4 <-
    data.frame(
      mse_y1 = mse(
        brasileirao_2021_with_pred$y1,
        brasileirao_2021_with_pred$y1_pred_m4
      ),
      mse_y2 = mse(
        brasileirao_2021_with_pred$y2,
        brasileirao_2021_with_pred$y2_pred_m4
      )
    )
  ...
  log_lik_m4 <-
    extract_log_lik(m4_fit, parameter_name = "log_lik")
  r_eff_m4 <-
    relative_eff(exp(log_lik_m4), chain_id = rep(1, 2500))
  loo_m4 <- loo(log_lik_m4, r_eff = r_eff_m4)
  waic_m4 <- loo(log_lik_m4)
  
  # Model 5 ----
  
  brasileirao_2021_list_m5 <- list(
    G = nrow(brasileirao_2021),
    T = 20,
    h = brasileirao_2021$h,
    a = brasileirao_2021$a,
  ...
  m5 <-
    stan_model(file = "models/modelo2-poisson-bivariate.stan",
               model_name = "model3-bivariate-gamma11")
  
  m5_fit <- sampling(
    object = m5,
    data = brasileirao_2021_list_m5,
    chains = 1,
    iter = 5000
  )
  ...
  traceplot_m5 <-
    traceplot(m5_fit, parameters_m5, nrow = 5, ncol = 2)
  
  ggsave(
    filename = paste0(path_to_save, "traceplot_m5.png"),
    plot = traceplot_m5,
    width = 9.46,
    height = 6.27,
    dpi = 300
  ...
  y_predict_m5 <-
    extract(m5_fit, pars = c("y1_tilde", "y2_tilde"))
  y_predict_m5_df <-
    data.frame(
      y1_pred_m5 = y_predict_m5$y1_tilde[1, ],
      y2_pred_m5 = y_predict_m5$y2_tilde[1, ]
    )
  
  brasileirao_2021_with_pred <-
    cbind(brasileirao_2021_with_pred, y_predict_m5_df)
  
  score_brasileirao_2021_m5_pred <-
    get_team_points_per_game(
      data = brasileirao_2021_with_pred,
      y1 = quo(y1_pred_m5),
      y2 = quo(y2_pred_m5)
  ...
  ggsave(
    filename = paste0(path_to_save, "cumsum_m5.png"),
    plot = cumsum_m5,
    width = 9.76,
    height = 7.37,
    dpi = 300
  ...
  mse_m5 <-
    data.frame(
      mse_y1 = mse(
        brasileirao_2021_with_pred$y1,
        brasileirao_2021_with_pred$y1_pred_m5
      ),
      mse_y2 = mse(
        brasileirao_2021_with_pred$y2,
        brasileirao_2021_with_pred$y2_pred_m5
      )
    )
  ...
  log_lik_m5 <-
    extract_log_lik(m5_fit, parameter_name = "log_lik")
  r_eff_m5 <-
    relative_eff(exp(log_lik_m5), chain_id = rep(1, 2500))
  loo_m5 <- loo(log_lik_m5, r_eff = r_eff_m5)
  waic_m5 <- waic(log_lik_m5)
  
  # Compare all models ----
  
  points_per_game_obs_m1_m2 <- merge(
  ...
  points_per_game_obs_m3_m4 <- merge(
  ...
  cumsum_all <- points_per_game_obs_all_models |>
    ...
    create_cumsum_points_plot(linewidth = 0.8, year = 2021) +
  ...
  ggsave(
    filename = paste0(path_to_save, "cumsum_all.png"),
    plot = cumsum_all,
    width = 9.76,
    height = 7.37,
    dpi = 300
