...
path_to_save <- "artifacts/2020/"

time_ids <-
  fread("data/times_ids.csv") |> distinct(id, .keep_all = TRUE)
...
brasileirao_2020_list <- list(
  G = nrow(brasileirao_2020),
  T = 20,
...
brasileirao_2020_with_pred <-
  cbind(brasileirao_2020, y_predict_m1_df)
...
mse_m1 <- data.frame(
  mse_y1 = mse(
    brasileirao_2020_with_pred$y1,
    brasileirao_2020_with_pred$y1_pred_m1
  ),
  mse_y2 = mse(
    brasileirao_2020_with_pred$y2,
...
brasileirao_2020_with_pred <-
  cbind(brasileirao_2020_with_pred, y_predict_m2_df)
...
brasileirao_2020_with_pred <-
  cbind(brasileirao_2020_with_pred, y_predict_m3_df)
...
ggsave(
  ...
  brasileirao_2020_with_pred <-
    cbind(brasileirao_2020_with_pred, y_predict_m4_df)
  
  score_brasileirao_2020_m4_pred <-
    get_team_points_per_game(
      data = brasileirao_2020_with_pred,
      y1 = quo(y1_pred_m4),
      y2 = quo(y2_pred_m4)
    )
  ...
  brasileirao_2020_with_pred <-
    cbind(brasileirao_2020_with_pred, y_predict_m5_df)
